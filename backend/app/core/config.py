...
class Settings(BaseSettings):
    ...
    # Game economics (all configurable via admin panel)
    EGG_VALUE_USD: float = 0.20          # $ per egg per day per chicken
    FEED_COST_USD: float = 0.18          # $ per feed bag
    CHICKEN_COST_USD: float = 1.0
    GLOVE_COST_USD: float = 2.0
    SUPER_CHICKEN_COST_USD: float = 3.0
    DOG_COST_USD: float = 5.0
    AUTO_FEEDER_COST_PERCENT: float = 3.0  # 3% of balance
    ...
    REFERRAL_FEED_BONUS: int = 3         # feed bags per referral (early bonus)
